...
func (c *Client) noder(ctx context.Context, table string, id xid.ID) (Noder, error) {
	switch table {
	case domain.Table:
		query := c.Domain.Query().
			Where(domain.ID(id))
		if fc := graphql.GetFieldContext(ctx); fc != nil {
			if err := query.collectField(ctx, true, graphql.GetOperationContext(ctx), fc.Field, nil, domainImplementors...); err != nil {
				return nil, err
			}
		}
...
func (c *Client) noders(ctx context.Context, table string, ids []xid.ID) ([]Noder, error) {
	...
	case domain.Table:
		query := c.Domain.Query().
			Where(domain.IDIn(ids...))
		query, err := query.CollectFields(ctx, domainImplementors...)
		if err != nil {
			return nil, err
		}
