...
func (dc *DomainCreate) AddActiveUserIDs(ids ...xid.ID) *DomainCreate {
	dc.mutation.AddActiveUserIDs(ids...)
	return dc
}

// AddActiveUsers adds the "active_users" edges to the User entity.
func (dc *DomainCreate) AddActiveUsers(u ...*User) *DomainCreate {
	ids := make([]xid.ID, len(u))
	for i := range u {
		ids[i] = u[i].ID
	}
	return dc.AddActiveUserIDs(ids...)
}

// AddRoleIDs adds the "roles" edge to the Role entity by IDs.
func (dc *DomainCreate) AddRoleIDs(ids ...xid.ID) *DomainCreate {
	dc.mutation.AddRoleIDs(ids...)
	return dc
}

// AddRoles adds the "roles" edges to the Role entity.
func (dc *DomainCreate) AddRoles(r ...*Role) *DomainCreate {
...
func (dc *DomainCreate) check() error {
	if _, ok := dc.mutation.Name(); !ok {
		return &ValidationError{Name: "name", err: errors.New(`dbauth: missing required field "Domain.name"`)}
	}
	return nil
}
...
func (dc *DomainCreate) createSpec() (*Domain, *sqlgraph.CreateSpec) {
	...
	if nodes := dc.mutation.ActiveUsersIDs(); len(nodes) > 0 {
...
func (u *DomainUpsertOne) Exec(ctx context.Context) error {
	if len(u.create.conflict) == 0 {
		return errors.New("dbauth: missing options for DomainCreate.OnConflict")
	}
...
func (u *DomainUpsertOne) ID(ctx context.Context) (id xid.ID, err error) {
	if u.create.driver.Dialect() == dialect.MySQL {
		// In case of "ON CONFLICT", there is no way to get back non-numeric ID
		// fields from the database since MySQL does not support the RETURNING clause.
		return id, errors.New("dbauth: DomainUpsertOne.ID is not supported by MySQL driver. Use DomainUpsertOne.Exec instead")
	}
...
func (u *DomainUpsertOne) IDX(ctx context.Context) xid.ID {
	id, err := u.ID(ctx)
	if err != nil {
		panic(err)
	}
...
func (u *DomainUpsertBulk) Exec(ctx context.Context) error {
	...
	for i, b := range u.create.builders {
		if len(b.conflict) != 0 {
			return fmt.Errorf("dbauth: OnConflict was set for builder %d. Set it on the DomainCreateBulk instead", i)
		}
	}
	if len(u.create.conflict) == 0 {
		return errors.New("dbauth: missing options for DomainCreateBulk.OnConflict")
	}
	return u.create.Exec(ctx)
}
