...
func (rq *RoleQuery) QueryDomain() *DomainQuery {
...
func (rq *RoleQuery) QueryUserDomain() *UserDomainQuery {
...
func (rq *RoleQuery) Clone() *RoleQuery {
	...
	return &RoleQuery{
		...
		withUserDomain: rq.withUserDomain.Clone(),
...
func (rq *RoleQuery) WithDomain(opts ...func(*DomainQuery)) *RoleQuery {
	query := (&DomainClient{config: rq.config}).Query()
...
func (rq *RoleQuery) WithUserDomain(opts ...func(*UserDomainQuery)) *RoleQuery {
	query := (&UserDomainClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	rq.withUserDomain = query
	return rq
}
...
func (rq *RoleQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Role, error) {
	...
	if query := rq.withDomain; query != nil {
		if err := rq.loadDomain(ctx, query, nodes, nil,
			func(n *Role, e *Domain) { n.Edges.Domain = e }); err != nil {
			return nil, err
		}
	}
	if query := rq.withUserDomain; query != nil {
		if err := rq.loadUserDomain(ctx, query, nodes,
			func(n *Role) { n.Edges.UserDomain = []*UserDomain{} },
			func(n *Role, e *UserDomain) { n.Edges.UserDomain = append(n.Edges.UserDomain, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (rq *RoleQuery) loadDomain(ctx context.Context, query *DomainQuery, nodes []*Role, init func(*Role), assign func(*Role, *Domain)) error {
...
func (rq *RoleQuery) loadUserDomain(ctx context.Context, query *UserDomainQuery, nodes []*Role, init func(*Role), assign func(*Role, *UserDomain)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(userdomain.FieldRoleID)
	}
	query.Where(predicate.UserDomain(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(role.UserDomainColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (rq *RoleQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := rq.ctx.Fields; len(fields) > 0 {
		...
		if rq.withDomain != nil {
			_spec.Node.AddColumnOnce(role.FieldDomainID)
		}
	}
