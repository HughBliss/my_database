...
func (eq *ExampleQuery) FirstID(ctx context.Context) (id xid.ID, err error) {
	var ids []xid.ID
...
func (eq *ExampleQuery) FirstIDX(ctx context.Context) xid.ID {
...
func (eq *ExampleQuery) OnlyID(ctx context.Context) (id xid.ID, err error) {
	var ids []xid.ID
...
func (eq *ExampleQuery) OnlyIDX(ctx context.Context) xid.ID {
...
func (eq *ExampleQuery) IDs(ctx context.Context) (ids []xid.ID, err error) {
...
func (eq *ExampleQuery) IDsX(ctx context.Context) []xid.ID {
	ids, err := eq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (eq *ExampleQuery) Exist(ctx context.Context) (bool, error) {
	...
	case IsNotFound(err):
		return false, nil
	case err != nil:
		return false, fmt.Errorf("dbexample: check existence: %w", err)
	default:
		return true, nil
	}
}
...
//		GroupBy(example.FieldSomeString).
//		Aggregate(dbexample.Count()).
//		Scan(ctx, &v)
...
//
//	client.Example.Query().
//		Select(example.FieldSomeString).
//		Scan(ctx, &v)
...
func (eq *ExampleQuery) prepareQuery(ctx context.Context) error {
	for _, inter := range eq.inters {
		if inter == nil {
			return fmt.Errorf("dbexample: uninitialized interceptor (forgotten import dbexample/runtime?)")
		}
	...
	for _, f := range eq.ctx.Fields {
		if !example.ValidColumn(f) {
			return &ValidationError{Name: f, err: fmt.Errorf("dbexample: invalid field %q for query", f)}
		}
	}
	if eq.path != nil {
...
func (eq *ExampleQuery) Modify(modifiers ...func(s *sql.Selector)) *ExampleSelect {
...
// Modify adds a query modifier for attaching custom logic to queries.
func (es *ExampleSelect) Modify(modifiers ...func(s *sql.Selector)) *ExampleSelect {
